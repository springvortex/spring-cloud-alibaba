...
```
spring-cloud-alibaba
├── service-common      公共模块，存放 DTO、统一响应、常量等，所有微服务依赖
...
4. `service-consumer` / `service-admin`（按需）
...
Entity（`com.zjc.provider.entity`）映射数据库表，仅模块内部使用，不对外暴露。
...
业务配置（数据源、MyBatis-Plus 等）存放在 Nacos，按 `group=服务名` 分组。
