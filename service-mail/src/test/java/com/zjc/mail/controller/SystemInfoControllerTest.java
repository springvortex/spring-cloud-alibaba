...
class SystemInfoControllerTest {
    ...
    private BuildProperties mockBuildProperties() {
        ...
        props.setProperty("name", "Service Mail");
        props.setProperty("version", "1.0.0");
        props.setProperty("artifact", "service-mail");
        props.setProperty("group", "com.zjc");
        props.setProperty("description", "邮件服务，提供统一收发邮件能力，记录入库");
    ...
    void testInfoWithBuildProperties() {
        ...
        assertThat(resp.getData().getProjectName()).isEqualTo("Service Mail");
        assertThat(resp.getData().getVersion()).isEqualTo("1.0.0");
        assertThat(resp.getData().getArtifact()).isEqualTo("service-mail");
    }
